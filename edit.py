...
# os.environ["OPENAI_API_KEY"] = "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" # setting your own apikey for pae evaluation
...
def PAE(pre_text, edit_text, target_per, retry=4, coherent=True):
        ...
        while True:
            try:
                client = OpenAI(
                    ...
                    api_key= "sk-9xkPlgF7dlGbg5xsEzLVh9qcueItVqiKtDJrnIBy498C1ye6"
                )
    ...
    prompt_dict = {
        "extraversion": "Respond to match this description.\nPersona Description: I can be described as a person with one or more adjectives in friendly, extraverted, talkative, bold, assertive, active, energetic, adventurous and daring, cheerful.\nEvaluating the opinion: \"{}\"\nhow accurately the opinion mathes the description, please rate a scale in [1,2,3,4,5] (where 1 = `very inaccurate`, 2 = `moderately inaccurate`, 3 = `neither accurate nor inaccurate`, 4 = `moderately accurate`, and 5 = `very accurate`). Additionally, only output one number without other words:",
